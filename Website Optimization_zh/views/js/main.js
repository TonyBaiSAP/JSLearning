...
function updatePositions() {
  ...
  var initPosition = !!arguments[0] ? arguments[0] : 0;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((scrollTop / 1250) + (i % 5));
    items[i].style.transform = 'translateX(' + 500 * phase + initPosition + 'px)';
  }
...
window.addEventListener('scroll', function(){
  var initPosition = 400;
  requestAnimationFrame(updatePositions.bind(this, initPosition));
});
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var iCaculatedCount = (window.innerHeight / 100) * cols;
  for (var i = 0; i < iCaculatedCount; i++) {
